package DTS::Assignment::Destination;
...
C<DTS::Assignment::Destination> represents the destination string of a DTS DynamicPropertiesTaskAssignment object.
...
C<DTS::Assignment::Destination> is a "syntatic sugar" to allow the different types of Destination string to be 
used with a set of methods, hidding the complexity and hardwork to deal with this string.

C<DTS::Assignment::Destination> is a abstract class and it's not meant to be used directly: to instantiate objects, look
for the subclasses of it.

Although is part of the package, C<DTS::Assignment::Destination> is B<not> a subclass of C<DTS::Assignment>, so no 
...
Expects as an argument the Destination string as a parameter. Subclasses of C<DTS::Assignment::Destination> must 
...
This method must be overrided by subclasses of C<DTS::Assignment::Destination>.
...
This method tests which object is being changed by the C<DTS::Assignment::Destination> object.
Expects a object name as a parameter; returns true if it changes the same object name, false if not.

An valid object name is equal to one of the subclasses of C<DTS::Assignment::Destination>.
